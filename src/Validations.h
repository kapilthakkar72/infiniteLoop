...
bool isPosOccupiedByOtherPlayer(Player player, Position pos, Graph graph_struct) {
	if (player.playerNumber == PlayerNum_P1
			&& graph_struct.graph[pos.row][pos.col] == ObjectType_PLAYER2)
		return true;

	if (player.playerNumber == PlayerNum_P2
			&& graph_struct.graph[pos.row][pos.col] == ObjectType_PLAYER1)
		return true;
